...
window.initKonsernKontroll = async (userId?: string | number, demoMode?: boolean) => {
  ...
  try {
    ...
    const renderAppForGroup = async (selectedGroupId: number) => {
      ...
      try {
        const accessRes = await getNEON({ table: 'usercompanyaccess', where: { user_id: rawUser.id } });
      ...
      const mappedCompanies = rawCompanies.map((c: any) => {
        ...
        return {
          ...
          publicFeesDate: c.public_fees_date || c.publicFeesDate || '',
          salaryExpensesDate: c.salary_expenses_date || c.salaryExpensesDate || '',
          shortTermDebtDate: c.short_term_debt_date || c.shortTermDebtDate || '',
