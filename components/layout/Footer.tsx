    ...
    <div className="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-slate-800/95 backdrop-blur border-t border-slate-200 dark:border-slate-700 shadow-[0_-4px_20px_-5px_rgba(0,0,0,0.1)] z-20 transition-colors duration-300">
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-center">
            ...
            <div className="overflow-x-auto whitespace-nowrap scrollbar-hide w-full flex justify-start md:justify-center items-center pb-1 sm:pb-0">
                    ...
                    {showLoyaltyBonus && totalLoyaltyBonusYTD > 0 && <MetricChip label="Loj.Bonus" value={totalLoyaltyBonusYTD} bgClass="bg-amber-50/50 border-amber-100/50 dark:bg-amber-900/10 dark:border-amber-800/50" textClass="text-amber-600 dark:text-amber-400" />}
                    {visibleFields.resultat && <MetricChip label="Resultat" value={totalResult - (showLoyaltyBonus ? totalLoyaltyBonusYTD : 0)} bgClass="bg-indigo-50/50 border-indigo-100/50 dark:bg-indigo-900/10 dark:border-indigo-800/50" textClass="text-indigo-600 dark:text-indigo-400" />}
